...
public class SQLDatabaseHelper extends SQLiteOpenHelper {
	 ...
	 * @param	baseSQLModel	An instance of a class that extends BaseSQLModel, 
	 * 							it is used to get the columnType map associated with the 
	 * @return
	 */
	@SuppressWarnings("unchecked")
	public static <T> T[] retreiveSQLSelectResults(Class<T> c, Cursor cursor, BaseSQLModel baseSQLModel) {
		if (c.isAssignableFrom(BaseSQLModel.class)) {
			throw new IllegalArgumentException("Only classes that extends BaseSQLModel can be populated with SQLite cursor results");
		}
		
		HashMap<String, Integer> columnTypeMap = baseSQLModel.getModelColumnTypeMap();
